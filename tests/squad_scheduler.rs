...
async fn full_lifecycle_mocked_evaluator_writes_workflow_and_records_run() {
    ...
    let evaluator = Arc::new(MockEvaluator {
        ...
        respond: |request: &EvaluationRequest| {
            ...
            EvaluationOutcome::WorkflowExecuted {
                reason: Some("the condition was met".to_string()),
                workflow_path,
    ...
    // The leader verdict's reason travels onto the run history.
    let runs = store.runs_for("issue-triage", 10).unwrap();
    assert_eq!(runs[0].reason.as_deref(), Some("the condition was met"));
}
...
async fn not_triggered_path_records_not_triggered_and_generates_no_workflow() {
    ...
    let evaluator = Arc::new(MockEvaluator {
        ...
        respond: |_request: &EvaluationRequest| EvaluationOutcome::NotTriggered { reason: None },
    });
