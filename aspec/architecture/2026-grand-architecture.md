# awman grand architecture refactor april 2026

## Purpose
awman has become a spaghetti implementation of 3 different modes: CLI, TUI, and API. Each of these 3 modes is meant to provide an identical set of functionality (core business logic) with 3 unique presentation layers (CLI for scripting, TUI for human interaction, API for API operation). What has instead arisen is a codebase where each of the 3 frontends provide a patchwork of function calls into a smattering of internal crates with a vague similarity in what they do without any guarantees of functional parity.

This grand refactor will aim to solve that once and for all. awman will be re-designed into a layered architecture that will guarantee codebase health for the forseeable future and allow for simpler implementation of future frontend modes such as desktop apps, code editor extensions, kubernetes operators, and more. The health and viability of the awman project depends on this refactor being successful, as without it there will be no way to offer users a consistent and high-quality experieince using the tool.

## Concept
The new layered architecture will ensure that core business logic, data structures, storage, and container runtime operations are completely seperated from the frontend modalities. Each of the 3 frontends (CLI, TUI, API) will become simple presentation layers atop a robust set of core engines. Each presentation layer will be only responsible for output (in the form of stdout/stderr for CLI, rendered TUI, and API responses for API) and input (stdin for CLI, keyboard input for TUI, and API requests for API). This refactor will make it impossible for the core behaviour of awman to drift between the 3 modalities, and will guarantee that any future frontends will not need to re-implement any of the core business logic.
...
2) NONE of the frontend packages (TUI, CLI, or API) may EVER implement any business logic. ALL business logic MUST be consumed via the layer below, the `Command` layer. The frontend layer's sole responsibility is displaying outputs and recieving inputs. It should NEVER attempt to control the flow of business logic EXCEPT where the command layer requires a frontend trait to manage inputs and outputs. 
...
- A new KEY, CORE data concept: `Session`. This will replace both `TabState` (as a TUI tab will just become a frontend representation of a `Session`) and the session concept in the API server.
  - `Session` and one of its internal structures, `SessionState` will be the new RULING TYPE for all awman operations. `Session` includes the working directory and git root, which EVERY command must respect, it includes information about the default agent, all available agents, all relevant configuration (repo- and global-config), and stores `SessionState`. `SessionState` will store all information related to the command being executed, any workflow state that is currently being run, the current container being used, any relevant error states, and anything else essential to the ongoing execution of commands and workflows in the session.
  - Every single command must be instantiated with a `Session`, and it will be the core guiding light for all command and workflow execution. Even the CLI, which will now be "just another frontend", which can only support a single session at at time, will use `Session` to guide its operations whereas before it would infer state from whichever directory it was launched from. CLI will be a single-Session frontend whereas TUI and API will be multi-session frontends. 
...
- All filesystem concerns, including (but not limited to):
  ...
  - Handling the API mode sqlite database
  - Handling the API mode directories
...
- Any and all other interactions between awman and the filesystem or databases it uses
...
- The `AgentRuntimeEngine` trait family (`src/engine/agent_runtime/`), responsible for any and all interactions with agent-isolation runtimes. It abstracts over the two paradigms awman supports and is the only runtime surface Layer 2 programs against (`Arc<dyn AgentRuntimeEngine>`):
  - Two concrete runtime tiers implement the trait:
    - `ContainerRuntime` (`src/engine/container/`) — the container-class tier (Docker, Apple Containers). It defines its own internal `ContainerBackend` trait, with `DockerBackend` / `AppleBackend` as concrete drivers. Container-paradigm-specific operations (image builds, local image probes, background setup/teardown containers) remain inherent methods on the concrete type and deliberately do NOT appear on the cross-paradigm trait — code that needs them must hold a typed `Arc<ContainerRuntime>`.
    - `SandboxRuntime` (`src/engine/sandbox/`) — the sandbox-class tier (microVM-per-session runtimes such as Docker Sandboxes). It defines its own internal `SandboxBackend` trait, with `DSbxBackend` as the first concrete driver. Sandbox-paradigm option types (`SandboxOption` / `ResolvedSandboxOptions`) mirror the container tier's.
  - All three tiers — the trait family, the concrete runtimes, and the backend drivers — live at Layer 1. Layer 2 never pattern-matches on concrete runtime types; paradigm-specific decisions branch on `AgentRuntimeEngine::capabilities()` (or `runtime_name()` for simple boolean dispatches). This is an instance of Tenet 3: typed objects and a builder/factory surface instead of raw pub functions.
  - The runtime engine is completely disaggregated from any consumer and is totally agnostic to whatever UX is put in front of it. As discussed in the tenets above, it provides agent constructors which take typed "options" (carried across the paradigm boundary as the `ResolvedAgentOptions` enum — `Container(ResolvedContainerOptions)` or `Sandbox(ResolvedSandboxOptions)`) to build an `AgentInstance` which can then be executed by a higher layer and provided with an `AgentFrontend` trait impl to handle inputs and outputs. Things like overlays, seeded prompts, interactivity, entrypoint commands, images/kits to be used, etc. are passed as option objects to a SMALL NUMBER of builders; `AgentInstance::run_with_frontend(some_frontend_trait)` then executes the configured agent using the provided frontend (which could be a pty, a stdin/stdout binding, etc.). This two-step build/run split is load-bearing: configure-time side-effects (image checks, overlay validation) happen between `build()` and `run_with_frontend()`.
  - This package includes an `AgentExecution` type which allows a fully-prepared agent run (produced by `run_with_frontend` above) to be provided to another package without leaking the details of the frontend itself. This way, `AgentExecution::wait` can be called and a package like `WorkflowEngine` below does not need to concern itself with how the agent's execution was prepared.
  - Runtime selection happens in one place: `agent_runtime::detect(&GlobalConfig)` maps the `runtime:` config string to a concrete tier (`docker` / unset → Docker; `apple-containers` → Apple Containers; `docker-sbx-experimental` → Docker Sandboxes) and returns both the cross-paradigm handle and the typed paradigm-specific handle. Unknown strings are a fatal `EngineError::UnknownRuntime` — never a silent Docker fallback (the user asked for an isolation model awman can't identify): the CLI prints the invalid value plus the valid set and exits; the TUI presents the same message in a startup modal whose only action is quit (Enter).

- The `WorkflowEngine`, responsible for any and all execution of awman workflows:
  ...
  - `WorkflowEngine` should NOT create `AgentInstance` itself, but CAN be given an `AgentExecution` by a higher-level caller to execute a workflow step with a given pre-configured agent.
  - `WorkflowEngine` DOES need to concern itself with exit codes etc. from agents, so things like `AgentExecution` will need to expose whatever outputs are relevant to the workflow, and still allow those outputs to flow appropriately upwards to higher-level callers with error wrapping.
...
- The `GitEngine`, responsible for any and all Git operations that awman requires, including (but not limited to):
  - Git root resolution
...
- The `AuthEngine`, responsible for any and all auth-related concerns including (but not limited to):
  ...
  - Handling authentication logic for the API server
  - Any and all other authentication concerns across awman

### Layer 2: command
Layer 2 is responsible for the higher-level business logic of operating awman's various commands.

Each command that awman provides, such as `chat`, `exec prompt`, `init`, etc. should all be implemented, in their entirety, within this package. No command or flag definitions, busines logic, flows, error handling, etc. should leak into higher layers.

Key components:

- A new `Dispatch` package which is used to route inputs (such as command strings from the TUI, or command execution requests received from the API API) to appropriate command types and methods.
...
- TUI frontend:
  - Launched with bare `awman` command, as always
...
- API frontend
  - Responsible for all API server operations including binding ports, handling auth and TLS, etc.
  ...
  - No validation of requests should reside in the API frontend package, all of that logic must be handled by lower-level packages like Dispatch
...
- The `awman` binary entrypoint lands here, builds the `clap` command structure, then executes it. That's it. Frontends and other lower-level packages handle everything else.

## Summary
To conclude, this architecture re-work is THE MOST IMPORTANT undertaking in the history of the awman project. It is extremely important that this refactor goes well to ensure the longevity and success of the project.
