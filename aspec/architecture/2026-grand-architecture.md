...
- The `AgentRuntimeEngine` trait family (`src/engine/agent_runtime/`), responsible for any and all interactions with agent-isolation runtimes. It abstracts over the two paradigms awman supports and is the only runtime surface Layer 2 programs against (`Arc<dyn AgentRuntimeEngine>`):
  ...
  - Runtime selection happens in one place: `agent_runtime::detect(&GlobalConfig)` maps the `runtime:` config string to a concrete tier (`docker` / unset → Docker; `apple-containers` → Apple Containers; `docker-sbx-experimental` → Docker Sandboxes; unknown strings warn and fall back to Docker) and returns both the cross-paradigm handle and the typed paradigm-specific handle.
